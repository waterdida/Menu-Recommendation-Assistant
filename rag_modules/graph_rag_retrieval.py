            ...
            with self.driver.session() as session:
                ...
                if graph_query.query_type == QueryType.MULTI_HOP:
                    ...
                    AND ANY(kw IN $target_keywords WHERE
                        ANY(prop IN [target.name, target.category, target.cuisineType, target.tags, target.preferredTerm, target.fsn]
                            WHERE prop IS NOT NULL AND (toString(prop) CONTAINS kw OR kw CONTAINS toString(prop)))
                    )"""
                    ...
                    MATCH (source)
                    WHERE (source:Recipe OR source:Ingredient OR source:Category OR source:RecipeCategory)
                      AND ANY(prop IN [source.name, source.category, source.cuisineType, source.tags, source.preferredTerm, source.fsn]
                          WHERE prop IS NOT NULL AND (toString(prop) CONTAINS source_name OR source_name CONTAINS toString(prop)))
                    
                    // 执行多跳遍历
                    MATCH path = (source)-[*1..{max_depth}]-(target)
                    WHERE NOT source = target
                      AND (target:Recipe OR target:Ingredient OR target:Category OR target:RecipeCategory OR target:CookingStep)
                      AND ALL(r IN relationships(path) WHERE NOT type(r) IN ['HAS_CONCEPT_TYPE', 'DIFFICULTY_LEVEL']){target_filter_clause}
                    ...
                    // 路径评分：短路径 + 关系类型匹配 + 业务节点优先，避免元概念节点抢分
                    WITH path, source, target, path_len, rels, path_nodes,
                         (1.0 / path_len) + 
                         (CASE WHEN ANY(r IN rels WHERE type(r) IN $relation_types) THEN 0.5 ELSE 0.0 END) +
                         (CASE WHEN target:Ingredient THEN 0.7 WHEN target:Recipe THEN 0.4 WHEN target:Category OR target:RecipeCategory THEN 0.1 ELSE 0.0 END) +
                         (REDUCE(s = 0.0, n IN path_nodes | s + COUNT {{ (n)--() }}) / 100.0 / size(path_nodes)) as relevance
    ...
    def extract_knowledge_subgraph(self, graph_query: GraphQuery) -> KnowledgeSubgraph:
        ...
        try:
            with self.driver.session() as session:
                ...
                WHERE (source:Recipe OR source:Ingredient OR source:Category OR source:RecipeCategory)
                  AND ANY(prop IN [source.name, source.category, source.cuisineType, source.tags, source.preferredTerm, source.fsn]
                      WHERE prop IS NOT NULL AND (toString(prop) CONTAINS entity_name OR entity_name CONTAINS toString(prop)))
                
                // 获取指定深度的邻居
                MATCH path = (source)-[*1..{graph_query.max_depth}]-(neighbor)
                WHERE (neighbor:Recipe OR neighbor:Ingredient OR neighbor:Category OR neighbor:RecipeCategory OR neighbor:CookingStep)
                  AND ALL(rel IN relationships(path) WHERE NOT type(rel) IN ['HAS_CONCEPT_TYPE', 'DIFFICULTY_LEVEL'])
                WITH source,
                     collect(DISTINCT neighbor) as neighbors,
                     reduce(allRels = [], rels IN collect(relationships(path)) | allRels + rels) as relationships
        ...
        try:
            ...
            for rel in record["rels"]:
                relationships.append({
                    "type": getattr(rel, "type", type(rel).__name__),
                    "properties": dict(rel)
                })
