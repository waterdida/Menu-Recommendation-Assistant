...
class IntelligentQueryRouter:
    ...
    def analyze_query(self, query: str) -> QueryAnalysis:
        ...
        try:
            ...
            analysis = QueryAnalysis(
                ...
                recommended_strategy=self._parse_strategy(result.get("recommended_strategy")),
                confidence=result.get("confidence", 0.5),
                reasoning=result.get("reasoning", "默认分析")
            )

            analysis = self._apply_rule_guardrail(query, analysis)
    ...
    def _rule_based_analysis(self, query: str) -> QueryAnalysis:
        ...
        reasoning_keywords = [
            "为什么", "为何", "原因", "影响", "关系", "联系", "关联", "区别", "比较",
            "对比", "替代", "相似", "适合", "能不能", "可不可以", "怎么搭", "怎么配"
        ]
        relation_keywords = [
            "配", "搭配", "组合", "相关", "联系", "连接", "一起", "替换", "相克",
            "适合", "类似", "同类"
        ]
        broad_search_keywords = ["推荐", "哪些", "有什么", "有啥", "列出", "找", "筛选"]
        constraint_keywords = [
            "并且", "同时", "要求", "限制", "不超过", "以内", "低糖", "少油", "低脂",
            "减脂", "糖尿病", "素食", "快手", "分钟"
        ]

        reasoning_hits = sum(1 for kw in reasoning_keywords if kw in normalized_query)
        relation_hits = sum(1 for kw in relation_keywords if kw in normalized_query)
        broad_hits = sum(1 for kw in broad_search_keywords if kw in normalized_query)
        constraint_hits = sum(1 for kw in constraint_keywords if kw in normalized_query)

        complexity = min((reasoning_hits + constraint_hits * 0.75 + broad_hits * 0.5) / 3.0, 1.0)
        relation_intensity = min(relation_hits / 2.0, 1.0)

        if broad_hits and (constraint_hits or relation_hits):
            strategy = SearchStrategy.COMBINED
        elif reasoning_hits or relation_hits:
        ...
        return QueryAnalysis(
            ...
            reasoning_required=bool(reasoning_hits or relation_hits),
            entity_count=max(1, len(re.findall(r"[\w\u4e00-\u9fff]+", query))),
    ...
    def _parse_strategy(self, strategy: Optional[str]) -> SearchStrategy:
        """将LLM策略名安全转换为枚举，异常时回到传统检索。"""
        try:
            return SearchStrategy(strategy or SearchStrategy.HYBRID_TRADITIONAL.value)
        except ValueError:
            logger.warning(f"未知检索策略: {strategy}，降级为传统混合检索")
            return SearchStrategy.HYBRID_TRADITIONAL

    def _apply_rule_guardrail(self, query: str, analysis: QueryAnalysis) -> QueryAnalysis:
        """规则兜底保护：明显需要图关系的问题不被LLM误压回传统检索。"""
        rule_analysis = self._rule_based_analysis(query)
        if rule_analysis.recommended_strategy == SearchStrategy.HYBRID_TRADITIONAL:
            return analysis
        ...
        analysis.recommended_strategy = rule_analysis.recommended_strategy
        analysis.query_complexity = max(analysis.query_complexity, rule_analysis.query_complexity)
        analysis.relationship_intensity = max(
            analysis.relationship_intensity,
            rule_analysis.relationship_intensity
        )
        analysis.reasoning_required = analysis.reasoning_required or rule_analysis.reasoning_required
        analysis.confidence = max(analysis.confidence, rule_analysis.confidence)
        analysis.reasoning = f"{analysis.reasoning}；规则保护触发：{rule_analysis.reasoning}"
        return analysis
